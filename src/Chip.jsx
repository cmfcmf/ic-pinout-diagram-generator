        ...
        <table>
          <tbody>
            {nTimes(variant.pins.length / 2).map((i) => {
              ...
              let leftFirstTagIndex = pinLeft.tags.findIndex(
                (each) => each !== null
              );
              ...
              let rightLastTagIndex = findLastIndex(
                pinRight.tags,
                (each) => each !== null
              );
              ...
              return (
                <tr key={i}>
                  {alignData ? (
                    <>
                      {pinLeft.tags.map((tag, i) =>
                        tag === null ? (
                          <td
                            key={i}
                            className={i >= leftFirstTagIndex ? "empty" : ""}
                          />
                        ) : (
                          <td key={i} className="badge" style={tag.style}>
                            {tag.value}
                          </td>
                        )
                      )}
                    ...
                    <td style={{ textAlign: "right" }}>
                      <div className="dense">
                        {pinLeft.tags
                          .filter((tag) => tag !== null)
                          .map((tag, i) => (
                            <div key={i} className="badge" style={tag.style}>
                              {tag.value}
                            </div>
                          ))}
                      </div>
                  ...
                  {alignData ? (
                    <>
                      {pinRight.tags.map((tag, i) =>
                        tag === null ? (
                          <td
                            key={i}
                            className={i < rightLastTagIndex ? "empty" : ""}
                          />
                        ) : (
                          <td key={i} className="badge" style={tag.style}>
                            {tag.value}
